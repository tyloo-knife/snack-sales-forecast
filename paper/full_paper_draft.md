...
问题四在门店-商品粒度上融合前三问信息。综合 Ridge 的特征包括滞后销量、滚动均值、星期、月份、是否周末、门店、商品、类别、天气、节假日和活动日。模型可写为：

$$
\hat y_{s,p,t}=f(Lag_{s,p,t},Roll_{s,p,t},Cal_t,Store_s,Product_p,Category_p,External_t).
$$

其中，$Lag$ 包括 `lag_1`、`lag_7`、`lag_14`，$Roll$ 包括 `rolling_mean_7` 和 `rolling_mean_14`。Ridge 回归用于可解释的综合线性预测；随机森林和其他树模型只作为补充比较，不作为论文主模型。
...
未来 7 天综合 Ridge 条件预测中，各门店预测总销量最高的是 `B地江山店店` 321.801，其次为 `C地五乡镇店` 279.972 和 `A地天九街店` 233.438；各类别预测最高的是 `包装散称` 443.881，其次为 `功能饮料` 374.886 和 `碳酸饮料` 216.494。完整门店-商品-日期小数预测表见 `outputs/final_7day_forecast.csv`；按“件数”提交时使用整数化副本 `outputs/final_7day_forecast_integer.csv`。

图 12 `figures/q4_store_forecast_7day_total.png` 用于展示未来 7 天门店预测总量；图 13 `figures/q4_category_forecast_7day_total.png` 用于展示类别预测总量；图 14 `figures/q4_error_by_store_bar.png` 和图 15 `figures/q4_error_by_category_bar.png` 用于定位误差较高的门店和类别；图 16 `figures/q4_actual_vs_predicted_scatter.png` 用于检查严格递推验证中预测值与真实值的对应关系。

误差诊断表明，模型在高销量商品组 WAPE=19.23%，低销量商品组 WAPE=33.07%；门店 `A地双桥路店` WAPE=116.93%，说明低销量门店或稀疏序列的百分比误差会显著放大。该结论与 `outputs/intermittent_demand_review.md` 中“54 条门店-商品序列零销量比例不低于 50%”一致。
...
不确定性主要来自四方面：外部情景假设、低销量序列误差放大、稀有天气样本不足、以及模型选择与验证样本重叠。若正式提交要求整数销量，本文另生成 `outputs/final_7day_forecast_integer.csv`：预测值小于 0 的先置为 0，其余按四舍五入转为整数；原始小数预测表不覆盖，继续作为模型输出依据。
...
最终结论是：简单可解释模型在本题中具有很强竞争力。问题四综合 Ridge 虽然在日滚动一步预测和部分聚合层级上有数值优势，但在严格 7 日递推主粒度下未超过问题一门店-商品简单指数平滑强 baseline。因此，本文不夸大综合模型的预测提升，而将其作为融合历史销量、类别和外部变量的综合解释模型。最终预测表可继续使用 `outputs/final_7day_forecast.csv`，但其含义是给定历史同期天气和活动日情景下的条件预测；整数化提交副本为 `outputs/final_7day_forecast_integer.csv`。
