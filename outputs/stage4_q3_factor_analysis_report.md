...
问题四进一步对天气、温度、风力做了预测敏感性检验。检验采用相同的严格 7 日递推验证窗口，即 2022-03-01 至 2022-03-28 的 4 个完整周窗口，并保持 Ridge 模型结构一致，只改变天气相关变量的处理方式。
...
该结果说明，天气、温度、风力在问题四预测中具有一定补充贡献：完整外部变量模型相对去天气模型 WAPE 下降约 1.04 个百分点；历史同期天气情景模型没有明显劣化。但这一结论仍属于预测敏感性证据，不是因果证据。论文中应写为“天气、温度、风力可作为外部情景变量改善预测”，不能写为“天气导致销量变化”。
...
6. 天气、温度、风力对预测有补充价值，但未来天气不可知时必须写明情景假设，并保留去天气模型作为稳健性对照。
