...
| --- | --- | --- |
| 2022-03-29 | 2022-04-04 | 不足完整 7 日销售或外部变量覆盖，不能作为严格 7 日递推窗口 |
...
| 综合随机森林 | 1.767 | 3.533 | 74.185 | 5432.000 | 5408.476 | 2280 |
...
日滚动一步预测口径下，综合模型中验证 WAPE 最低的是 `综合Ridge回归`。与问题一门店-商品简单指数平滑相比，门店-商品层级 WAPE 相对改进为 2.68%。
...
| 综合随机森林 | 2.023 | 4.908 | 85.073 | 5061.000 | 5788.576 | 2128 |

严格递推口径下，综合模型中 WAPE 最低的是 `综合Ridge回归`，门店-商品 WAPE 为 76.68%。严格 7 日递推下，`综合Ridge回归` 的门店-商品 WAPE 比日滚动一步口径高 3.67 个百分点，说明误差在递推过程中有所累积。

## 11. 日滚动一步预测与严格递推预测对比

| model_label | daily_WAPE_pct | recursive_WAPE_pct | WAPE_change_pct_points | WAPE_relative_change_pct | recursive_result |
| --- | --- | --- | --- | --- | --- |
| 问题一门店-商品简单指数平滑 | 75.633 | 76.346 | 0.713 | 0.943 | 严格递推误差更高 |
| 综合Ridge回归 | 73.013 | 76.684 | 3.671 | 5.028 | 严格递推误差更高 |
| Baseline滚动均值(14日) | 75.787 | 77.657 | 1.870 | 2.468 | 严格递推误差更高 |
| Baseline移动平均(7日) | 76.171 | 78.429 | 2.258 | 2.965 | 严格递推误差更高 |
| 综合随机森林 | 74.145 | 85.073 | 10.928 | 14.739 | 严格递推误差更高 |
...
解释：检验使用每日绝对误差总和作为配对序列。若 p 值小于 0.05，表述为验证误差下降具有统计证据；否则只能表述为“数值上下降/未能证明显著下降”。当前门店-商品层级结论为：达到配对t检验意义上的显著改进。
