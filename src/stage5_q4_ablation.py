...
问题四正文以“完整综合 Ridge + 严格递推消融”说明综合模型确实整合了前三问信息；但最终模型优劣表述应降调为：综合 Ridge 在门店、商品等聚合层面有较好解释价值，在门店-商品最细粒度上未超过问题一简单指数平滑。消融实验适合放在模型评价或附录，用来证明团队没有简单堆特征，而是做了可复现的方法取舍。
"""
